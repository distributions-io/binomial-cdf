...
* @param {Number} [opts.n=1] - number of trials
...
function cdf( x, options ) {
	...
	if ( isMatrixLike( x ) ) {
		...
		return cdf5( out, x, opts.n, opts.p );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.n, opts.p );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.n, opts.p, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.n, opts.p, opts.accessor );
		}
		return cdf2( out, x, opts.n, opts.p );
	}
	return NaN;
} // end FUNCTION cdf()
